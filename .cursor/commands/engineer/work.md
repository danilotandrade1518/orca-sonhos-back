...
- TL;DR: Code Standards (index), Backend Architecture (index)
- Sem docs longos, sem Top-3 trechos, sem Jira
- Respeita Ignore List

**Modo Standard (≤60k tokens)**:

- TL;DR adicionais: `backend-architecture`, `code-standards`, `testing-strategy`
...
- `documentation.json`, `coverage/**`, `dist/**`, `node_modules/**`, `sessions/*/work-log.md`, `temp/*-context-inventory.md`, `*.js.map`

## Objetivo

Implementar a funcionalidade seguindo o plano faseado, com foco na qualidade, padrões do projeto (Clean Architecture + DDD + CQRS) e aprovação entre etapas.
...
const phaseRegex =
  /^##\s*📅\s*FASE\s*(\d+)\s*:.*?\[\s*Status\s*:\s*([^\]]+)\]/gim;
...
function normalizeStatus(raw: string): PhaseStatus {
  ...
  if (
    ...
    return 'completed';
  if (s.includes('⏳') || s.includes('pendente') || s.includes('pending'))
    return 'pending';
  return 'unknown';
}
...
let current: PhaseInfo | undefined = phases.find(
...
if (!current) {
  ...
  if (lastCompleted) {
    current = phases.find(
      (p) => p.num > lastCompleted.num && p.status !== 'completed',
    );
  }
  if (!current) {
    current =
      phases.find((p) => p.status === 'pending' || p.status === 'unknown') ??
      phases.at(-1);
  }
}
...
– Metaspecs são lidas via cache por hash/TTL; se inalterados, reutilize TL;DR em `temp/context-cache/`.
...
function checkTokenLimit(): boolean {
  if (tokenCount >= INTERRUPT_THRESHOLD) {
    console.log(
      `⚠️ Token limit atingido: ${tokenCount}/${HARD_CAP}. Interrompendo carregamentos.`,
    );
    return false;
  }
  return true;
...
1. **Busca Contextual Inteligente Guiada por Demanda**:
   ...
   const contextQuery = `funcionalidade ${folderName} arquitetura padrões backend clean architecture DDD`;
...
- **NOVO**: Com base na fase identificada, direcione a busca para seções específicas (ex: fase de testes → testes, performance, observabilidade).
...
const metaSpecsPath =
  extractMetaSpecsPath(aiProperties) ||
  'https://github.com/danilotandrade1518/orca-sonhos-meta-specs';
...
const backendArchIndex = await read_file({
  target_file: `${metaSpecsPath}/technical/backend-architecture/index.md`,
});
tokenCount += estimateTokens(backendArchIndex);
if (!checkTokenLimit()) return;
...
function getRelevantSectionsForPhase(
  ...
  if (
    phaseStatus === 'testes' ||
    phaseStatus === 'validação' ||
    phaseNumber === totalPhases
  ) {
    sections.push('technical/04_estrategia_testes.md');
    sections.push('technical/backend-architecture/observability.md'); // Observabilidade
    sections.push('technical/backend-architecture/performance.md'); // Performance
  }

  // Fases de desenvolvimento (meio do plano)
  if (phaseNumber >= 3 && phaseNumber < totalPhases) {
    sections.push('technical/code-standards/typescript-patterns.md');
    sections.push('technical/backend-architecture/clean-architecture.md');
    sections.push('technical/backend-architecture/ddd-patterns.md');
  }
...
// 5. Carregar apenas seções relevantes com contabilização
for (const section of relevantSections) {
  const content = await read_file({
    target_file: `${metaSpecsPath}/${section}`,
  });
...
// 6. ANÁLISE CONTEXTUAL INTELIGENTE
await performIntelligentAnalysis({
  metaSpecsIndex: metaSpecsIndex,
  codeStandardsIndex: codeStandardsIndex,
  backendArchIndex: backendArchIndex,
  featureContext: featureAnalysis,
});
```

- Cacheie o TL;DR das Meta Specs em `temp/context-cache/backend-patterns.tldr.md` (TTL configurável) e referencie-o sem imprimir conteúdo completo.
...
- [ ] **Navegar pelos índices** de code-standards e backend-architecture
...
- [ ] **Identificar padrões TypeScript** e Clean Architecture
...
const featureContext = await analyzeFeatureContext({
  ...
  useCaseType: await identifyUseCaseType(),
  aggregateType: await identifyAggregateType(),
...
const similarUseCases = await codebase_search({
  query: searchQueries.useCasePatterns,
  target_directories: ['src/application/use-cases/'],
});

const similarAggregates = await codebase_search({
  query: searchQueries.aggregatePatterns,
  target_directories: ['src/domain/aggregates/'],
});

const similarRepositories = await codebase_search({
  query: searchQueries.repositoryPatterns,
  target_directories: ['src/application/contracts/repositories/'],
});
...
**Padrões Backend**: [Padrões Clean Architecture/DDD identificados]
...
**Padrões de Error Handling**: [Either pattern e tratamento de erros]
```
...
const appliedPatterns = await applyIdentifiedPatterns({
  ...
  backendArch: backendArchAnalysis,
  existingPatterns: similarUseCases,
  featureContext: featureAnalysis,
});
...
const consistencyCheck = await validateConsistency({
  ...
  existingCodebase: similarUseCases,
  architecturalRules: backendArchAnalysis,
});
```
...
- [ ] Usar padrões de import corretos (path aliases: @domain, @application, etc.)
- [ ] Implementar error handling com Either pattern
- [ ] Aplicar padrões TypeScript modernos

**Arquitetura**:

- [ ] Seguir Clean Architecture (Domain → Application → Infrastructure → Interface)
- [ ] Respeitar responsabilidades das camadas
- [ ] Implementar Use Cases conforme padrão IUseCase<TRequest>
- [ ] Aplicar DDD (Agregados, Value Objects, Domain Events)
- [ ] Seguir CQRS (Commands vs Queries)
- [ ] Implementar Composition Roots para DI

**Domain Layer**:

- [ ] Criar agregados seguindo padrão AggregateRoot
- [ ] Implementar Value Objects quando aplicável
- [ ] Validar regras de negócio no domínio
- [ ] Usar DomainError para erros de domínio

**Application Layer**:

- [ ] Implementar Use Cases com IUseCase<TRequest>
- [ ] Retornar Either<DomainError | ApplicationError, UseCaseResponse>
- [ ] Criar DTOs para entrada de dados
- [ ] Definir contratos de repositórios em contracts/repositories
- [ ] Implementar serviços de aplicação quando necessário

**Infrastructure Layer**:

- [ ] Implementar repositórios concretos
- [ ] Configurar adaptadores de banco de dados
- [ ] Implementar autenticação/autorização
- [ ] Configurar observabilidade (logs, métricas)

**Interface Layer**:

- [ ] Criar controllers HTTP
- [ ] Implementar mappers (DTO ↔ Domain)
- [ ] Configurar rotas
- [ ] Implementar middlewares necessários
...
- [ ] Implementar testes de integração quando aplicável
- [ ] Seguir padrões de teste do projeto (.spec.ts para unitários, .test.ts para integração)

#### Passo 3: Busca e Atualização do Jira

**_Este passo só deve ser feito se o trabalho ainda não iniciou. Verifique o status do plano para esta informação. Caso o plano já esteja em andamento, ou seja, se alguma fase já iniciou, ignore este passo._**

**Fluxo Automático**:

1. **Buscar Task**: Use `mcp_atlassian_search` com o nome da pasta da sessão
2. **Identificar Transições**: Use `mcp_atlassian_getTransitionsForJiraIssue` para encontrar transição "Em Progresso"
3. **Atualizar Status**: Use `mcp_atlassian_transitionJiraIssue` para fazer a transição
...
### 5.5. Validações de API e Backend

**Durante implementação de funcionalidades backend, valide:**

#### Clean Architecture Compliance

- [ ] Camadas respeitadas (Domain → Application → Infrastructure → Interface)
- [ ] Dependências apontam para dentro (Interface → Application → Domain)
- [ ] Domain não depende de outras camadas
- [ ] Use Cases implementam IUseCase<TRequest>
- [ ] Repositórios definidos como contratos na Application

#### DDD Patterns

- [ ] Agregados criados seguindo AggregateRoot
- [ ] Value Objects implementados quando aplicável
- [ ] Regras de negócio no domínio
- [ ] DomainError para erros de domínio
- [ ] Entidades com identificadores únicos

#### Error Handling

- [ ] Either pattern usado consistentemente
- [ ] DomainError vs ApplicationError diferenciados
- [ ] Erros mapeados corretamente para HTTP
- [ ] Mensagens de erro apropriadas

#### API Design

- [ ] Rotas RESTful quando aplicável
...
- [ ] Queries otimizadas
- [ ] Logs estruturados
- [ ] Métricas implementadas quando necessário
...
- **Linting/Formatação**: Execute `npm run lint` e `npm run format`
- **Testes Automatizados**: Execute `npm run test:unity`, `npm run test:integration`, `npm run test:e2e`
- **Code Analysis**: Use ferramentas MCP para análise de qualidade
- **Documentation**: Consulte Context7 para APIs de bibliotecas
- **Migrations**: Use `npm run migrate:create` para criar migrações
...
10. **🏗️ Clean Architecture**: Respeite sempre as camadas e dependências
11. **🎯 DDD**: Mantenha regras de negócio no domínio
12. **⚡ CQRS**: Separe comandos de queries quando aplicável
