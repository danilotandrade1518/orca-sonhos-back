...
describe('EntityName', () => {
  ...
  it('deve retornar erro se o nome for vazio', () => {
    ...
    expect(name.errors[0]).toBeInstanceOf(InvalidEntityNameError);
    expect(name.errors[0].errorObj.field).toBe('name');
    expect(name.errors[0].errorObj.message).toMatch(/invalid/i);
  });
