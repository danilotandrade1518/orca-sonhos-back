  ...
  implements IUseCase<UpdateEnvelopeDto>
{
  ...
  async execute(
    ...
    if (typeof dto.name !== 'undefined') {
      const nameResult = envelope.updateName(dto.name);
      if (nameResult.hasError) {
        return Either.errors(nameResult.errors);
      }
    }

    if (typeof dto.monthlyLimit !== 'undefined') {
      const limitResult = envelope.updateLimit(dto.monthlyLimit);
      if (limitResult.hasError) {
        return Either.errors(limitResult.errors);
      }
    }
